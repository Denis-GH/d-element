...
export class FilterManager {
  constructor({ filterName, onUpdate, filterCfg, debounceDelayForInput = 1000 }) {
    ...
    this.container = document.querySelector(`[${this.attrs.filterContainer}="${filterName}"]`);
  ...
  handleEvent(event) {
    ...
    const changeData = {
      value: target.type === "checkbox" || target.type === "radio" ? null : target.value, // Для чекбоксов и радио кнопок значение null
      isChecked:
  ...
  applyFilters(filtersCfg) {
    ...
    Object.entries(inputs).forEach(([name, data]) => {
      ...
      // Устанавливаем значения в зависимости от типа элемента
      switch (filterItem.type) {
        case "checkbox":
        case "radio":
          filterItem.checked = !!data.isChecked;
          break;
        case "text":
          filterItem.value = data.value || "";
          break;
        default:
          if (filterItem.tagName === "INPUT") {
            filterItem.value = data.value || "";
          } else {
            console.warn(`Тип элемента фильтра "${filterItem.tagName}" не поддерживается.`);
          }
      }
    });
  }
